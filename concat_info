...
df = pd.read_csv('./reviews_Hospital_info_치과.csv', index_col=0)
print(df.info())
df.drop_duplicates()
...
df.columns = ['names', 'clinic','address','link','telephone']
#df.to_csv('./reviews_Hospital_치과.csv')

for i in range(1, 19):
    df_temp = pd.read_csv(f'./reviews_Hospital_info_{titles[i]}.csv', index_col=0)
    df.drop_duplicates()
...
print(df.info())
df.to_csv('./total_reviews_Hospital_info.csv', encoding='utf-8-sig')
